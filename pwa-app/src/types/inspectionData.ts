...
export interface InspectionEvaluation {
  id: string;
  eval: StandardEvaluation | ManagementEvaluation | LegalEvaluation | 'freetext' | string;
  memo: string; // 場所メモ
  concernDetail?: string; // 違法性関係の懸念内容（後方互換性のため残す）
  legalDocValue?: number; // 遵法性 資料値(m)
  legalMeasuredValue?: number; // 遵法性 実測値(m)
  freetextContent?: string; // 自由入力内容（違法性関係の懸念内容・不適合箇所）
  surveyMethods?: SurveyMethod[]; // 調査方法（複数選択可）
  rebarPitch?: number; // No.96 配筋のピッチ(cm)
  schmidtValues?: number[]; // No.97 シュミットハンマー測定値(9回)
  schmidtResult?: number; // No.97 算定結果
...
export interface InspectionItem {
  ...
  evalType: 'standard' | 'management' | 'legal' | 'rebar' | 'schmidt' | 'freetext' | 'select';
...
// カテゴリごとの調査実施状況
export type SurveyState = 'conducted' | 'not_required' | 'not_conducted';
export interface CategorySurveyStatus {
  conducted: boolean; // true: 調査実施, false: 調査実施不可（後方互換）
  surveyState?: SurveyState; // 3択: 実施 / 実施不要 / 不可
...
export interface PropertyInspectionData {
  ...
  // 調査実施状況（キーはcategoryId（cat4, cat5）またはgroupId（group_yukashita等）が混在）
  categorySurveyStatus?: {
    [categoryOrGroupId: string]: CategorySurveyStatus;
  };

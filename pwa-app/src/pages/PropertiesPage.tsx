...
import { loadData, updateData, logout } from '../storage/indexedDB';
...
const PropertiesPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log('[PropertiesPage] Loading data from IndexedDB...');
      const data = await loadData();
      console.log('[PropertiesPage] Data loaded:', {
        ...
        defectsCount: data.defects.length,
      });
      if (!data.currentUser) {
        console.log('[PropertiesPage] No current user, redirecting to login');
        navigate('/');
        return;
      }
  ...
  const handleAddProperty = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('物件追加エラー:', error);
      alert('物件の追加に失敗しました: ' + (error instanceof Error ? error.message : String(error)));
      setShowAddModal(false);
    }
  };
  ...
  const handleDeleteProperty = async (id: string) => {
    if (window.confirm('この物件を削除してもよろしいですか？')) {
      ...
      await updateData('floors', data.floors);
      await updateData('referenceImages', data.referenceImages);
    }
  };
